DROP TABLE IF EXISTS public.ship_predicted_positions_open_sea_florida;

CREATE TABLE public.ship_predicted_positions_open_sea_florida AS
WITH coded AS (
    SELECT
        spp.*,
        CASE
    ...
    FROM public.ship_predicted_positions_florida spp
),
labeled AS (
    SELECT
        c.id,
    ...
    WHERE EXISTS (
        SELECT 1
        FROM public.water_polygons_bufer_florida wp
        WHERE ST_Intersects(
            c.geom,
...
CREATE INDEX IF NOT EXISTS idx_ship_predicted_positions_open_sea_florida_geom
ON public.ship_predicted_positions_open_sea_florida
USING GIST (geom);

CREATE INDEX IF NOT EXISTS idx_ship_predicted_positions_open_sea_florida_mmsi
ON public.ship_predicted_positions_open_sea_florida ("MMSI");

CREATE INDEX IF NOT EXISTS idx_ship_predicted_positions_open_sea_florida_api_id
ON public.ship_predicted_positions_open_sea_florida (api_id);

CREATE INDEX IF NOT EXISTS idx_ship_predicted_positions_open_sea_florida_sensing_time
ON public.ship_predicted_positions_open_sea_florida (sensing_time_without_tz);

CREATE INDEX IF NOT EXISTS idx_ship_predicted_positions_open_sea_florida_target_type
ON public.ship_predicted_positions_open_sea_florida ("Target ship type");

ANALYZE public.ship_predicted_positions_open_sea_florida;
